...
function addItem(frm) {
	...
	//create a food item or misc item based on what was types into the "item type text box"
...
function workItem(category) {
	this.itemType = "Work Item";
	this.category = category;
}

function homeItem(category) {
	this.itemType = "Home Item";
	this.category = category;
}

function schoolItem(category) {
	this.itemType = "School Item";
	this.category = category;
}
...
function addItemToDom(itm) {
	...
	itemDiv.setAttribute("style","border:1px solid black; width:200px;height:150px;");
...
function saveList() {
	...
	console.log(typeof localStorage.savedList);
	console.log(typeof myItems);
	
	var visibileItems = document.getElementById("shoppingListDiv"); 
	if (visibileItems.style.display === "none") {
		visibileItems.style.display = "block"; 
		} else { 
		visibileItems.style.display = "none";
